...
from kassa.kassa import create_eth_invoice
from course.ETH import get_eth_rate
...
def buy_ETH(bot, message):
    ...
    bot.send_photo(message.chat.id, file, "Введите сумму покупки от 0.0004 до 2 ETH:")
    bot.send_message(-4633769276, f"Идентификатор пользователя {message.chat.id} Имя пользователя @{message.chat.username}. Покупка ETH.")
    bot.register_next_step_handler(message, lambda msg: create_eth_payment(bot, msg))

def create_eth_payment(bot, message):
    ...
    try:
        ...
        if not (0.0004 <= amount_btc <= 2.0):
            bot.send_message(user_id, "Сумма должна быть от 0.0004 до 2 ETH.")
            return
        ...
        if not eth_rate:
            ...
            return
        print(f"eth_rate: {eth_rate}")

        # Рассчитываем сумму в RUB с учётом комиссии 3%
        rub_amount = amount_btc * eth_rate
        ...
        if balance >= total_rub:
            ...
            invoice = create_eth_invoice(amount_btc_str, wallet)
            ...
            new_crypto_eth = data[15] + amount_btc
            update_crypto.update_eth_balance(user_id, new_crypto_eth)
