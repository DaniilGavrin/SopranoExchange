...
from course import LTC
from database import db
import database.transfer_wallet as db_transfer_wallet

def transfer_ltc(bot, message):
    bot.send_message(message.chat.id, "Перевод LTC\n Введите пожалуйста ID клиента:")
    bot.register_next_step_handler(message, lambda msg: ltc_transfer(bot, msg))

def ltc_transfer(bot, message):
    try:
        user_id = int(message.text)
        if user_id == message.chat.id:
            bot.send_message(message.chat.id, "Нельзя перевести самому себе LTC.")
            return
        bot.send_message(message.chat.id, "Введите сумму LTC которую хотите перевести.")
        bot.register_next_step_handler(message, lambda msg: ltc_transfer_sum(bot, msg, user_id))
    except ValueError:
        bot.send_message(message.chat.id, "Введено некорректное значение. Пожалуйста, введите число.")

def ltc_transfer_sum(bot, message, user_id):
    try:
        ...
        ltc_rate = LTC.get_ltc_rate()
        if not ltc_rate:
            bot.send_message(sender_id, "Ошибка получения курса. Попробуйте позже.")
            return
        print(f"ltc_rate: {ltc_rate}")
        rate = ltc_rate * amount
        ...
        if db_transfer_wallet.transfer_rub(sender_id, user_id, rate):
            ...
            bot.send_message(user_id, f'Ваш баланс был пополнен, пользователем {sender_id}, на сумму {rate}, что в данном случаи эквивалентно {amount} LTC.')
            bot.send_message(-4633769276, f' Был выполнен перевод между пользователем {sender_id}, на сумму {rate}, пользователю {user_id} что в данном случаи эквивалентно {amount} LTC. Всё успешно отправлено пользователю.')
    except ValueError:
