...
from course import ETH
from database import db
import database.transfer_wallet as db_transfer_wallet

def transfer_eth(bot, message):
    bot.send_message(message.chat.id, "Перевод ETH\n Введите пожалуйста ID клиента:")
    bot.register_next_step_handler(message, lambda msg: eth_transfer(bot, msg))

def eth_transfer(bot, message):
    try:
        user_id = int(message.text)
        if user_id == message.chat.id:
            bot.send_message(message.chat.id, "Нельзя перевести самому себе ETH.")
            return
        bot.send_message(message.chat.id, "Введите сумму ETH которую хотите перевести.")
        bot.register_next_step_handler(message, lambda msg: eth_transfer_sum(bot, msg, user_id))
    except ValueError:
        bot.send_message(message.chat.id, "Введено некорректное значение. Пожалуйста, введите число.")

def eth_transfer_sum(bot, message, user_id):
    try:
        ...
        eth_rate = ETH.get_eth_rate()
        if not eth_rate:
            bot.send_message(sender_id, "Ошибка получения курса. Попробуйте позже.")
            return
        print(f"sol_rate: {eth_rate}")
        rate = eth_rate * amount
        ...
        if db_transfer_wallet.transfer_rub(sender_id, user_id, rate):
            ...
            bot.send_message(user_id, f'Ваш баланс был пополнен, пользователем {sender_id}, на сумму {rate}, что в данном случаи эквивалентно {amount} ETH.')
            bot.send_message(-4633769276, f' Был выполнен перевод между пользователем {sender_id}, на сумму {rate}, пользователю {user_id} что в данном случаи эквивалентно {amount} ETH. Всё успешно отправлено пользователю.')
    except ValueError:
