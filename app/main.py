...
def get(message):
    ...
    bot.send_message(-4660555206, f'Была выполнена команда /get пользователем {user_id}')
...
def start(message):
    ...
    markup.add(button)
    bot.send_message(-4660555206, f'Была выполнена команда /start пользователем {user_id}')
...
def admin(message):
    ...
    if str(user_id) == settings.ADMIN_USER:
        ...
        bot.send_message(-4660555206, f'Внимание в панель администратора был выполнен заход с аккаунта \n{user_id}\n@{message.chat.username}')
        markup = types.InlineKeyboardMarkup()
        ...
        bot.send_message(-4660555206, f"Внимаение в панель администратора пытается зайти не администратор. Попытка была выполнена с аккаунта \n{user_id}\n@{message.chat.username}")


def set_trx(bot, message):
    user_id = message.chat.id
    file = open('Адрес.png', 'rb')
    bot.send_message(-4660555206, f'Пользователь {user_id} открыл функцию для сохранеия TRON кошелька')
...
def trx_user(message):
    ...
    bot.send_message(-4660555206, f'Пользователь {user_id} сохранил TRON кошелек, его адрес {trx}')
...
Установленный TRON (TRX) адрес: {trx_addr}""", reply_markup=prof)
    bot.send_message(-4660555206, f'Пользователь {user_id} открыл профиль')
...
def kurs(bot, message):
    ...
    bot.send_message(-4660555206, f'Пользователь {user_id} открыл курс валюты')
...
def contact(bot, message):
    ...
    bot.send_message(-4660555206, f'Пользователь {user_id} открыл контакты')

def accept_user(bot, message):
    print("1")
    user_id = message.chat.id
    bot.send_message(-4660555206, f'Пользователь {user_id} принял пользовательское соглашение')
...
def change_pay(bot, message):
    ...
    buy.add(npm1, npm2)
    bot.send_message(-4660555206, f'Пользователь {user_id} выбирает способ оплаты')
    bot.send_message(message.chat.id, 'Выберите способ оплаты', reply_markup=buy)

def buy(bot, message):
    user_id = message.chat.id
    file = open('Пополнение.png', 'rb')
    bot.send_message(-4660555206, f'Пользователь {user_id} выбрал crystalpay')
...
def buy2(bot, message):
    user_id = message.chat.id
    file = open('Пополнение.png', 'rb')
    bot.send_message(-4660555206, f'Пользователь {user_id} выбрал CryptoBot')
...
def create_payment_crb(message):
    ...
    bot.send_message(-4660555206, f'Пользователь {user_id} выполняет оплату. Выполняю проверку оплаты каждые 2 минуты. Идентификатор оплаты {result.get('invoice_id')}. имя пользователя @{message.chat.username}. В данный момент ручная проверка!! Платеж не будет зачислен автоматически!!!')
    if call.get('ok') == True:
        ...
        while items[0].get('status') == 'active':
            time.sleep(60)
            bot.send_message(-4660555206, f"Выполняется проверка оплаты пользователем. идентификатор оплаты {result.get('invoice_id')}. Статус оплаты {items[0].get('status')}")
            pay_time += 1
            if pay_time == 10:
                bot.send_message(user_id, "Время на оплату закончилось. Отменяем платеж.")
                bot.send_message(-4660555206, f"Оплата не прошла успешно. Время на оплату закончилось. Отменяем платеж.")
                pay_time = 0
                break
        call = Crypto.getInvoices(params={'invoice_ids': id})
        result = call.get('result')
        ...
        if items[0].get('status') == 'paid':
            ...
            bot.send_message(-4660555206, f"Оплата прошла успешно. Зачисляем деньги на баланс. Идентификатор оплаты {result.get('invoice_id')}. Деньги зачислим по курсу платежной системы.")
            ...
            for item in result:
                if item['source'] == 'TRX' and item['target'] == 'RUB':
                    ...
                    bot.send_message(-4660555206, f"Оплата прошла успешно. Зачисляем деньги на баланс. Идентификатор оплаты {result.get('invoice_id')}. Имя пользователя @{message.chat.username}. Деньги зачислим по курсу платежной системы. Зачислим пользователю {cost_comission}")
                    ...
                    if status_db == True:
                        ...
                        bot.send_message(-4660555206, f"Оплата прошла успешно. Зачислено {cost_comission} рублей на баланс. Комиссия {commission} рублей. Баланс обновлен. Идентификатор оплаты {result.get('invoice_id')}. имя пользователя @{message.chat.username}.")
                    else:
                        bot.send_message(-4660555206, f"Оплата прошла успешно, но в базе данных не удалось изменить баланс пользователя. Идентификатор оплаты {result.get('invoice_id')}. имя пользователя @{message.chat.username}. id пользователя {user_id}. Проверка выполняла в автоматическом режиме, и была успешной, проверьте базу данных!")
                    break
            else:
                rate = None  # Курс не найден
                bot.send_message(-4660555206, "Оплата была успешной, но в полатежкой системе не найден данный курс, проверьте логи и обратитесь к CryptoBot")
            ...
            bot.send_message(-4660555206, f'Пользователь {user_id} выполнял оплату. Оплата не была успешной. Идентификатор оплаты {result.get('invoice_id')}. имя пользователя @{message.chat.username}. Проверка выполняла в автоматическом режиме, и не была успешной, проверьте JSON ответ по данному платежу!')

    else:
        bot.send_message(user_id, "Оплата не была успешной, в платежкой системе ошибка. Обратитесь к администратору.")
        bot.send_message(-4660555206, f'Пользователь {user_id} выполнял оплату. Оплата не была успешной, в платежкой системе ошибка. Идентификатор оплаты {result.get('invoice_id')}. имя пользователя @{message.chat.username}. Проверка выполняла в автоматическом режиме, и не была успешной, проверьте JSON ответ по данному платежу!')
...
"""
    ...
    bot.send_message(-4660555206, f'Пользователь {user_id} выполняет оплату информация об оплате\n{payment_info}')
    ...
    if status.get('state') == 'paid':
        ...
        bot.send_message(-4660555206, f"Оплата прошла успешно. Зачисляем деньги на баланс. Имя пользователя @{message.chat.username}. Деньги зачислим по курсу платежной системы.")
        ...
        bot.send_message(-4660555206, f"Оплата не удалась. Идентификатор пользователя {message.chat.id} Имя пользователя @{message.chat.username}. Отменяем платеж.")

def buy_tron(bot, message):
    file = open('Покупка.png', 'rb')
    bot.send_photo(message.chat.id, file, "Введите сумму покупки от 20 рублей:")
    bot.send_message(-4660555206, f"Идентификатор пользователя {message.chat.id} Имя пользователя @{message.chat.username}. Покупка TRON.")
...
def create_tron_payment(message):
    ...
    try:
        ...
        if balance >= amount:
            if wallet != 0:
                ...
                if invoice != "3":
                    ...
                    bot.send_message(-4660555206, f"С пользователем {message.chat.id} не возникли проблемы.")
                else:
                    bot.send_message(user_id, 'Не удалось создать счет. Возникла проблема с кассой обратитесь к администратору')
                    bot.send_message(-4660555206, f"С пользователем {message.chat.id} возникли проблемы. Не удалось создать счет. На кассе нет средств для выполнения операции")
            else:
                bot.send_message(-4660555206, f"С пользователем {message.chat.id} возникли проблемы. Нет кошелька. Решаю вопрос автоматически")
        ...
        bot.send_message(-4660555206, f"С пользователем {message.chat.id} возникли проблемы. Не хватает баланса.")
...
def wallet_add(message):
    ...
    bot.send_message(-4660555206, f"Идентификатор пользователя {message.chat.id} Имя пользователя @{message.chat.username}. Кошелек сохранен в базу данных.")
    bot.register_next_step_handler(message, create_tron_payment)

def delete_user(bot, message):
    bot.send_message(-4660555206, "Запущена функция удаления пользователя администратором.")
...
def delete_user_id(message):
    ...
    if msg == "cancel":
        ...
        bot.send_message(-4660555206, "Удаление отменено администратором")
    else:
        try:
            user_id = message.chat.id
            db.delete_user(msg)
            bot.send_message(message.chat.id, f"Пользователь удален.")
            bot.send_message(-4660555206, f"Пользователь удален.")
        except ValueError:
            bot.send_message(message.chat.id, "Введите корректное числовое значение user_id.")
            bot.send_message(-4660555206, "Возникла ошибка при удалении пользователя")

def change_balance(bot, message):
    bot.send_message(-4660555206, "Запущена функция изменения баланса пользователя администратором.")
...
def change_balance_user_id(message):
    ...
    if msg == "cancel":
        ...
        try:
            ...
            bot.send_message(message.chat.id, "Введите корректное числовое значение user_id.")
            bot.send_message(-4660555206, "Возникла ошибка при изменении баланса пользователя")
...
def change_balance_new_balance(message, user_id):
    try:
        ...
        bot.send_message(-4660555206, f"Баланс пользователя с id {user_id} изменен на {new_balance}")
    except ValueError:
        bot.send_message(message.chat.id, "Введите корректное числовое значение баланса.")
        bot.send_message(-4660555206, "Возникла ошибка при изменении баланса пользователя")

def change_admin(bot, message):
    bot.send_message(-4660555206, "Запущена функция изменения администратора администратором.")
...
def change_admin_new_id(message):
    ...
    if msg == "cancel":
        ...
        bot.send_message(-4660555206, "Изменение отменено администратором")
    else:
        try:
            new_admin_id = msg
            settings.ADMIN_USER = new_admin_id
            bot.send_message(message.chat.id, f"Администратор изменен на id {new_admin_id}")
            bot.send_message(-4660555206, f"Администратор изменен на id {new_admin_id}")
        except ValueError:
            bot.send_message(message.chat.id, "Введите корректные данные")
            bot.send_message(-4660555206, "Возникла ошибка при изменении администратора")
